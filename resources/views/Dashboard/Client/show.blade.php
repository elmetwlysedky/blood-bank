...
@section('title')
    {{__('main.donation_requests')}} {{$client->name}}
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title">{{__('main.table')}} {{__('main.donation_requests')}} : {{$client->name}} : {{$client->donationRequest->count()}} </h5>
        ...
        <table class="table datatable-basic">
            ...
            <div>
                ...
                @php $counter = 1 @endphp
                 @isset($client->donationRequest)
                 @foreach($client->donationRequest as $item)
                     <tr>
                         <td>{{$counter++}}</td>
                         <td>{{$item->name}}</td>
                         <td>{{$item->phone}}</td>
